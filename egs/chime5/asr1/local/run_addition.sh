...
if [ $# != 3 ]; then
   ...
   echo "Usage: local/additiont.sh [options] <wav-in-dir> <wav-out-dir> <array-id>"
   echo "main options (for others, see top of script file)"
   echo "  --cmd <cmd>                              # Command to run in parallel with"
   echo "  --bmf \"1 2 3 4\"                        # microphones used for the sum"
   exit 1;
fi
...
chmod a+x $expdir/log/addition.*.sh
$cmd JOB=1:$nj $expdir/log/addition.JOB.log \
  $expdir/log/addition.JOB.sh
