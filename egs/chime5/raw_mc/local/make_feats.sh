...
if [ $# -lt 1 ] || [ $# -gt 3 ]; then
   ...
   echo "e.g.: $0 data/train exp/make_spectrogram/train mfcc"
...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_${feattype}_${name}.JOB.log \
    extract-segments scp,p:$scp $logdir/segments.JOB ark:- \| \
    local/capture.py --use-log-bank ${type} \| \
    copy-feats --compress=$compress $write_num_frames_opt ark:- \
     ark,scp:$spectrogramdir/raw_${feattype}_$name.JOB.ark,$spectrogramdir/raw_${feattype}_$name.JOB.scp \
     || exit 1;
#compute-spectrogram-feats $vtln_opts --verbose=2 --config=$spectrogram_config ark:- ark:-
  ...
  $cmd JOB=1:$nj $logdir/make_spectrogram_${name}.JOB.log \
    compute-spectrogram-feats $vtln_opts --verbose=2 --config=$spectrogram_config scp,p:$logdir/wav.JOB.scp ark:- \| \
    copy-feats --compress=$compress $write_num_frames_opt ark:- \
...
if [ -f $logdir/.error.$name ]; then
  ...
  tail $logdir/make_${feattype}_${name}.1.log
  exit 1;
fi

# concatenate the .scp files together.
for n in $(seq $nj); do
  cat $spectrogramdir/raw_${feattype}_$name.$n.scp || exit 1;
done > $data/feats.scp
