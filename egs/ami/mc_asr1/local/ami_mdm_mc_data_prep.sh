...
# find MDM mics
find $AMI_DIR -iname "*Array*.wav" | sort > $dir/wav.flist

n=`cat $dir/wav.flist | wc -l`
echo "In total, $n headset files were found."
[ $n -ne 1352 ] && \
  echo Warning: expected 1352 data data files, found $n
...
awk '{meeting=$1; channel="MDM"; speaker=$3; stime=$4; etime=$5;
 ...
 for(i=6;i<=NF;i++) printf(" %s", $i); printf "\n"}' $SEGS | sort | uniq > $dir/text_prev

rm -f $dir/text

for i in $(seq 1 8);
do
  sed 's/MDM/MDM.CH'"${i}"'-ARRAY/g' ${dir}/text_prev >> ${dir}/text
done
cat ${dir}/text > ${dir}/text_prev
sort ${dir}/text_prev > ${dir}/text
rm -f $dir/text_prev
...
sed -e 's?.*/??' -e 's?.wav??' $dir/wav.flist | \
 perl -ne 'split; $_ =~ m/(.*)\..*\-0([0-9])/;print "AMI_$1_MDM.CH$2-ARRAY\n"' | \
...
cat $dir/wav.scp | \
  perl -ane '$_ =~ m:^(\S+CH[0-9]-ARRAY).*\/([IETB].*)\.wav.*$: || die "bad label $_";
