...
find $AMI_DIR -iname "*Array*.wav" | sort > $tmpdir/wav.flist
...
awk '{meeting=$1; channel="MDM"; speaker=$3; stime=$4; etime=$5;
 ...
 for(i=6;i<=NF;i++) printf(" %s", $i); printf "\n"}' $SEGS | sort | uniq > $tmpdir/text_prev

rm -f $tmpdir/text

for i in $(seq 1 8);
do
  sed 's/MDM/MDM.CH'"${i}"'-ARRAY/g' ${tmpdir}/text_prev >> ${tmpdir}/text
done
cat ${tmpdir}/text > ${tmpdir}/text_prev
sort ${tmpdir}/text_prev > ${tmpdir}/text
