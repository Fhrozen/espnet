    ...
    def forward(self, x, h):
        ...
        for i in range(len(self.dilations)):
            output, skip = self._residual_forward(
                output, h, self.dil_sigmoid[i], self.dil_tanh[i],
                self.aux_1x1_sigmoid[i], self.aux_1x1_tanh[i],
                self.skip_1x1[i], self.res_1x1[i])
    ...
    def generate(self, x, h, n_samples, interval=None, mode="sampling"):
        ...
        for i, d in enumerate(self.dilations):
            output, _ = self._residual_forward(
                output, h_, self.dil_sigmoid[i], self.dil_tanh[i],
                self.aux_1x1_sigmoid[i], self.aux_1x1_tanh[i],
                self.skip_1x1[i], self.res_1x1[i])
        ...
        for i in range(n_samples):
            ...
            for i, d in enumerate(self.dilations):
                output, skip = self._generate_residual_forward(
                    ...
                    self.aux_1x1_sigmoid[i], self.aux_1x1_tanh[i],
                    self.skip_1x1[i], self.res_1x1[i])
                output = torch.cat([output_buffer[i], output], dim=2)
                output_buffer_next.append(output[:, :, -buffer_size[i]:])
                skip_connections.append(skip)
