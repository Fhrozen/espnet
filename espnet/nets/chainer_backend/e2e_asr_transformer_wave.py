...
from espnet.nets.chainer_backend.transformer_wave import ctc

from espnet.nets.chainer_backend.transformer_wave.attention import MultiHeadAttention
from espnet.nets.chainer_backend.transformer_wave.decoder import Decoder
from espnet.nets.chainer_backend.transformer_wave.encoder import Encoder
from espnet.nets.chainer_backend.transformer_wave.label_smoothing_loss import LabelSmoothingLoss
...
class E2E(ASRInterface, chainer.Chain):
    ...
    def recognize(self, x_block, recog_args, char_list=None, rnnlm=None):
        ...
        with chainer.no_backprop_mode(), chainer.using_config('train', False):
            ...
            batch = len(ilens)
            if (x_block.ndim == 2) and (x_block.shape[1] == 1):
                x_block = x_block[:, 0]
    ...
    def custom_converter(subsampling_factor=0):
        ...
        from espnet.nets.chainer_backend.transformer_wave.training import CustomConverter
        return CustomConverter()

    @staticmethod
    def custom_updater(iters, optimizer, converter, device=-1, accum_grad=1):
        """Get custom_updater of the model."""
        from espnet.nets.chainer_backend.transformer_wave.training import CustomUpdater
