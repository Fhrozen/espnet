...
class SegmentStreamingE2E(object):
    ...
    def __init__(self, e2e, recog_args, rnnlm=None):
        ...
        if not 'transformer' in str(type(e2e)):
    ...
    def accept_input(self, x):
        ...
        # TODO(nelson): Change for not HardCoded for Transformer
        ...
        # if not self._previous_encoder_recurrent_state is None:
        #     h = np.concatenate([self._previous_encoder_recurrent_state, h], axis=1)
        #     ilen = [h.shape[1]]
        # self._previous_encoder_recurrent_state = h
        h, _, _ = self._e2e.encoder(h, ilen)
        z = F.squeeze(self._e2e.ctc.argmax(h), axis=0).data
        logging.info(z)
        logging.info(self._char_list[z[0]])
        ...
        if self._activates == 1:
            self._encoder_states.extend(F.squeeze(h, axis=0).data)
            self._ctc_posteriors.extend(self._e2e.ctc.log_softmax(h)[0])
            ...
            if self._blank_dur >= self._recog_args.streaming_min_blank_dur:
                ...
                if seg_len > 0:
                    ...
                    h = F.concat(self._encoder_states[:seg_len], axis=0).reshape(
                        -1, self._encoder_states[0].shape[0])
                    if self._recog_args.ctc_weight > 0.0:
                        lpz = F.concat(self._ctc_posteriors[:seg_len], axis=0).reshape(
                            -1, self._ctc_posteriors[0].shape[0])
